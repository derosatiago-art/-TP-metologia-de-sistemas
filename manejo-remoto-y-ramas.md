...
Acá explicamos los comandos utilizados para trabajar en simultaneo sin romper el código original y para sincronizar nuestro trabajo con GitHub.

### `git branch`
Nos permite gestionar las ramas. Si lo ejecutamos solo, nos lista las ramas que tenemos. Si le agregamos un nombre (`git branch nombre-rama`), crea una rama nueva.

### `git checkout` (o `git switch`)
Se usa para "pasar" o saltar de una rama a otra. Por ejemplo, `git checkout main` nos devuelve a la rama principal. Si usamos `git checkout -b nombre-rama`, creamos una rama nueva y nos movemos a ella en un solo paso.
